...
def get_iou(mask_name,predict, image_mask=None):
...
def get_dice(mask_name,predict, image_mask=None):
    image_mask = cv2.imread(mask_name, 0)
    image_mask = cv2.resize(image_mask,(576,576))
...
# 计算像素准确率(PA)
def pixel_accuracy(mask_name, predict):
    ...
    output = (predict == image_mask).mean()
    return output

def get_hd(mask_name, predict, image_mask=None):
    image_mask = cv2.imread(mask_name, 0)
    image_mask = cv2.resize(image_mask,(576,576))
