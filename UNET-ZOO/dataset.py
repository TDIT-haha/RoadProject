...
class RoadSegDataset(data.Dataset):
    def __init__(self, state, transform=None, target_transform=None):
        self.state = state
        self.aug = True
        self.imagefolder = r"/root/project/Datas/otherDatas/dataset/images"
        self.maskfolder = r"/root/project/Datas/otherDatas/dataset/mask"
        self.trainText = r"/root/project/Datas/otherDatas/dataset/train.txt"
        # self.valText = r"/root/project/Datas/otherDatas/dataset/val.txt"#valall.txt
        self.valText = r"/root/project/Datas/otherDatas/dataset/valall.txt"#valall.txt
        self.testText = r"/root/project/Datas/otherDatas/dataset/test.txt"
    ...
    def getDataPath(self, ):
        ...
        self.val_img_paths = []
        self.val_mask_paths = []
        with open(self.trainText, "r") as ff:
            datas = ff.readlines()
        for datai in datas:
            datai = datai[:-1].split(" ")
            self.train_img_paths.append(os.path.join(self.imagefolder, datai[0]))
            self.train_mask_paths.append(os.path.join(self.maskfolder, datai[1]))

        with open(self.valText, "r") as ff:
            datas = ff.readlines()
        for datai in datas:
            datai = datai[:-1].split(" ")
            self.val_img_paths.append(os.path.join(self.imagefolder, datai[0]))
            self.val_mask_paths.append(os.path.join(self.maskfolder, datai[1]))
    ...
    def __getitem__(self, index):
        ...
        if self.transform is not None:
            img_x = self.transform(pic)
        if self.target_transform is not None:
            img_y = self.target_transform(mask)  #这个处理我感觉是有问题的，但是我先不追究了
        return img_x, img_y, pic_path, mask_path
