...
def getArgs():
    ...
    parse.add_argument('--dataset', default='roadseg',  # dsb2018_256
...
def getModel(args):
    ...
    if args.arch == 'UNet_Cbam':
        model = unet_cbam(3, 1).to(device)
    if args.arch == 'NestedUNet_Cbam':
...
def getDataset(args):
    ...
    if args.dataset == 'driveEye':
        ...
        test_dataloaders = DataLoader(test_dataset, batch_size=1)
    if args.dataset == 'roadseg':
        train_dataset = RoadSegDataset(r'train', transform=x_transforms, target_transform=y_transforms)
        train_dataloaders = DataLoader(train_dataset, batch_size=args.batch_size)
        val_dataset = RoadSegDataset(r"val", transform=x_transforms, target_transform=y_transforms)
        val_dataloaders = DataLoader(val_dataset, batch_size=1)
        test_dataset = RoadSegDataset(r"test", transform=x_transforms, target_transform=y_transforms)
        test_dataloaders = DataLoader(test_dataset, batch_size=1)
...
def test(val_dataloaders,save_predict=False):
    ...
    modelpath = r'./saved_model/'+str(args.arch)+'_'+str(args.batch_size)+'_'+str(args.dataset)+'_'+str(args.epoch)+'.pth'
    ...
    modelpath = r"/root/project/Modules/TrackAnomalyTask/UNET-ZOO/saved_model/NestedUNet_Cbam_6_roadseg_200.pth"
    model.load_state_dict(torch.load(modelpath, map_location='cpu'))  # 载入训练好的模型
    model.eval()
    ...
    with torch.no_grad():
        ...
        for pic,_,pic_path,mask_path in val_dataloaders:
            ...
            dice_total += dice
            pixels = pixel_accuracy(mask_path[0],predict)
            pixel_total += pixels
            ...
            #plt.pause(0.01)
            print('{}/{}: iou={}, dice={}, pa:{}'.format(i,num,iou,dice, pixels))
            if i < num:i+=1   #处理验证集下一张图
        #plt.show()
        print('Miou=%f,aver_hd=%f,dv=%f, mpa=%f' % (miou_total/num,hd_total/num,dice_total/num, pixel_total/num))
        logging.info('Miou=%f,aver_hd=%f,dv=%f, mpa=%f' % (miou_total/num,hd_total/num,dice_total/num, pixel_total/num))
...
if __name__ =="__main__":
    ...
    logging = getLog(args)
    print('**************************')
    print('models:%s,\nepoch:%s,\nbatch size:%s\ndataset:%s' % (args.arch, args.epoch, args.batch_size,args.dataset))
