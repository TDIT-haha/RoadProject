...
def json_to_xml(json_file, xml_file):
    ...
    folder.text = 'VOC2012'  # 替换为你自己的文件夹名称
    ...
    for shape in data['shapes']:
        if shape['shape_type'] == 'rectangle':  # 仅处理关键点类型的标注
            point = shape['points']
            ...
            xmin_node.text = str(int(point[0][0]))
            ymin_node = ET.SubElement(bndbox, 'ymin')
            ymin_node.text = str(int(point[0][1]))
            xmax_node = ET.SubElement(bndbox, 'xmax')
            xmax_node.text = str(int(point[1][0]))
            ymax_node = ET.SubElement(bndbox, 'ymax')
            ymax_node.text = str(int(point[1][1]))

    tree = ET.ElementTree(root)
    tree.write(xml_file, encoding='utf-8', xml_declaration=True)

if __name__ == '__main__':
    input_dir = 'E:/projects/project/seg/rail/paguma'  # 输入目录
    output_dir = 'E:/projects/project/seg/rail/voc'  # 输出目录
